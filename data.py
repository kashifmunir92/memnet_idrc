...
class Dataset(torchdata.Dataset):
    def __init__(self, data_path):
        ...
        self.d1, self.d2, self.d3, self.d4, self.d5, self.d6, self.d7 = torch.load(data_path)

    def __getitem__(self, index):
        return self.d1[index], self.d2[index], self.d3[index], self.d4[index], self.d5[index], self.d6[index], self.d7[index]
...
class Data(object):
    def __init__(self, use_cuda, conf, batch_size=None):
        ...
        if conf.corpus_splitting == 1:
            pre = localpath + '/data/processed/lin/'
        elif conf.corpus_splitting == 2:
            pre = localpath + '/data/processed/ji/'
        elif conf.corpus_splitting == 3:
            pre = localpath + '/data/processed/l/'
